...
//			cutText ["You are on the verge of dehydration.", "PLAIN DOWN"];
			cutText ["<t font='PuristaSemiBold' color='#04feff' size='2'>You are on the verge of dehydration!</t><br/>", "PLAIN DOWN", -1, true, true];
		};
		_rnd = ceil (random 8);
...
//		cutText ["You are on the verge of starvation.", "PLAIN DOWN"];
		cutText ["<t font='PuristaSemiBold' color='#ffb404' size='2'>You are on the verge of starvation!</t><br/>", "PLAIN DOWN", -1, true, true];
		};
		_rnd = ceil (random 8);
	...
	if (VoN_isOn) then {
		if ((VoN_currentTxt == "Alliance") or (VoN_currentTxt == "Outlaw") or (VoN_currentTxt == "Hunter") or (VoN_currentTxt == "Mayhem") or (VoN_currentTxt == localize "str_channel_group")) then 
		{
//			cutText ["Your faction cannot use faction voice! STOP OR YOU WILL GET KICKED!", "PLAIN DOWN"];
			cutText ["<t font='EtelkaNarrowMediumPro' color='#ffffff' size='3'>You cannot use faction voice! STOP OR GET KICKED!</t><br/>", "PLAIN DOWN", -1, true, true];
