...
switch _mode do {
	case "onLoad": {
		with (uinamespace) do {
			...
			//--- Mouse moving
			RscDisplayArcadeMap_Layout_6_mousemoving = {
			...
			_ctrlMap ctrladdeventhandler ["mousemoving","with uinamespace do {_this call RscDisplayArcadeMap_Layout_6_mousemoving};"];

			//--- Toolbox changed
			RscDisplayArcadeMap_Layout_6_ToolBoxSelChanged = {
				_id = _this select 1;
			...
			_ctrlToolbox ctrladdeventhandler ["ToolBoxSelChanged","with uinamespace do {[ctrlparent (_this select 0),_this select 1] call RscDisplayArcadeMap_Layout_6_ToolBoxSelChanged};"];
			[_display,0] call RscDisplayArcadeMap_Layout_6_ToolBoxSelChanged;

			_ctrlToolboxSection = _display displayctrl 1019;
			_ctrlToolboxSection ctrladdeventhandler ["ToolBoxSelChanged","with uinamespace do {['section',_this,''] call RscDisplayArcadeMap_Layout_6_script};"];
			...
			_control ctrladdeventhandler ["buttonclick","with uinamespace do {['collapse',_this,''] call RscDisplayArcadeMap_Layout_6_script};"];
	


			//--- Pressed keys
			RscDisplayArcadeMap_Layout_6_KeyDown = {
				_display = _this select 0;
				_key = _this select 1;

				#define TOOLBOX_CHANGE(ID)	[_display,ID] call RscDisplayArcadeMap_Layout_6_ToolBoxSelChanged;
				...
				switch _key do {
					...
					//--- Backspace
					//case 14: { [_display] call RscDisplayArcadeMap_Layout_6_HideButtonClicked; };	//click on hide button - done by shortcut in config
				};
				false
			};
			_display displayaddeventhandler ["keydown","with uinamespace do {_this call RscDisplayArcadeMap_Layout_6_KeyDown};"];
			
			
			//Disable buttons for PA-Lite
			if((productVersion select 1) == "Arma3AlphaLite") then		//No need to localize since PA-Lite has expired.
			{
				//Load
	...
	case "collapse": {
		...
		_y = [configfile >> "RscDisplayArcadeMap_Layout_6" >> "controlsBackground" >> "CA_Map","y"] call bis_fnc_returnconfigentry;
		_h = [configfile >> "RscDisplayArcadeMap_Layout_6" >> "controlsBackground" >> "CA_Map","h"] call bis_fnc_returnconfigentry;
		if (_enabled) then {
			_yCollapsed = [configfile >> "RscDisplayArcadeMap_Layout_6" >> "controlsBackground" >> "CA_Map","yCollapsed"] call bis_fnc_returnconfigentry;
			_hCollapsed = [configfile >> "RscDisplayArcadeMap_Layout_6" >> "controlsBackground" >> "CA_Map","hCollapsed"] call bis_fnc_returnconfigentry;
	...
	case "onUnload": {
		with (uinamespace) do {
			RscDisplayArcadeMap_Layout_6_mousemoving = nil;
			RscDisplayArcadeMap_Layout_6_ToolBoxSelChanged = nil;
			RscDisplayArcadeMap_Layout_6_KeyDown = nil;
		};
	};
};